...
import { formatDate, formatMoney, rangeDays, type DateRange, type PeriodPreset } from "@/utils/helper";
...
const day = (iso: string) => formatDate(`${iso}T12:00:00+05:00`);

export default function FinanceScreen({ report, preset, today }: FinanceScreenProps) {
  ...
  const navigate = (next: { preset: PeriodPreset; range?: DateRange }) => {
    ...
    router.push(`${routes.ui.finance}?${params.toString()}`);
  };

  const subtitle = range.from === range.to ? day(range.from) : `${day(range.from)} – ${day(range.to)} · ${days} days`;
  ...
  return (
    <>
      ...
      <div className="space-y-4 p-4">
        <PeriodPicker preset={preset} range={range} today={today} onChange={navigate} />
