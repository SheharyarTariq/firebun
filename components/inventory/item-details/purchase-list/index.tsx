...
interface PurchaseListProps {
  ...
  /** The whole item: the per-base-unit price needs displayUnit, which PackAware lacks. */
  item: InventoryItem;
}
            ...
            <div key={p.id} className={cn("px-4 py-3", voided && "opacity-60")}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {/* Its own line: both prices will not fit beside the quantity on a phone. */}
                  <p className={cn("text-xs text-muted tabular-nums", voided && "line-through")}>
                    {formatMoneyExact(perUnit)} / {unitName(p)}
                    {/* The per-base-unit price, unless it would repeat the figure beside it. */}
                    {p.enteredUnit !== item.displayUnit && (
                      <>
                        {" · "}
                        <span className="text-foreground">
                          {formatCostPerUnit(p.unitCost, item.displayUnit)}
                        </span>
                      </>
                    )}
                  </p>
